...
public class PowerBIService {
	 ...
	 */
	public static List<EmbedConfig> getReports(String reportId, String groupId, String accessToken) {
		...
		JSONArray reportJson = new JSONArray(responseBody);
		int reportsLength = reportJson!=null ? reportJson.length():0;
		for ( int i=0; i < reportsLength;i++ )
		{
			JSONObject responseObj = new JSONObject(reportJson.get(i));
			...
			reportEmbedConfig.name = responseObj.getString("name");
			reports.add(reportEmbedConfig);
		}
